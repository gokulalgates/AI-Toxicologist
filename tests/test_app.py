...
from core_logic import standardize_chemical_name, check_relevance, analyze_abstract_with_llm
from app import create_evidence_matrix
...
class TestApp(unittest.TestCase):
    ...
    def test_standardize_chemical_name(self):
        ...
        with patch('core_logic.pcp.get_compounds') as mock_get:
            mock_compound = Mock()
    ...
    def test_analyze_abstract_with_llm_dict_reasoning(self, mock_llm_class, mock_parser_class):
        ...
        # Mock the LLM
        mock_llm = Mock()
        # Ensure 'prompt | llm' returns the llm (or a mock that behaves like it)
        mock_llm.__ror__ = Mock(return_value=mock_llm)
        
        mock_response = Mock()
        mock_response.content = '{"kc1_status": "SUPPORTED", "reasoning": {"text": ["line 1", "line 2"]}}'
        mock_llm.invoke.return_value = mock_response
        mock_llm.return_value = mock_response  # In case chain calls __call__
        mock_llm_class.return_value = mock_llm
        ...
        mock_result.model_dump.return_value = {
            ...
            "reasoning": "line 1\nline 2", # The logic inside parse? No.
            # Wait, core_logic calls parse(), then model_dump().
            # logic to convert dict reasoning to string reasoning happens AFTER getting result?
            # Let's check core_logic.
        }
