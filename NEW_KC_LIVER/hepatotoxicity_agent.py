...
class HepatotoxicityAgent:
    ...
    def __init__(self, model_names=None, use_rag=True, rag_top_k=5, email="your.email@example.com", api_key=None):
        """
        ...
        if model_names is None:
            model_names = ["llama3.1:latest", "mistral:latest"]
        ...
        if use_rag:
            self.rag_system = RAGSystem(top_k=rag_top_k)
            self.pubmed_downloader = PubMedDownloader(email=email, api_key=api_key)
...
Respond in a clear, structured manner that helps users understand how their chemical or data maps to these Key Characteristics."""
    ...
    def _query_model(self, model_name: str, messages: List[Dict]) -> Optional[str]:
        """
    ...
    def chat(self, user_input: str) -> str:
        ...
        # Add assistant response to history (without context for cleaner history)
        self.chat_history.append({
            "role": "assistant",
            "content": assistant_message
        })
        
        # Remove context from history to keep it clean (context is in the user message)
        # Actually, we want to keep it for context, so leave as is
        
        return assistant_message
    
    def download_pubmed_abstracts(self, max_results=1000, email="your.email@example.com"):
        """
...
def main():
    ...
    if not email:
        print("Error: Email is required for NCBI API. Exiting.")
        exit(1)
