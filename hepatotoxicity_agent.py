...
try:
    ...
    try:
        ...
        rag_system_path = os.path.join(scripts_path, 'agent_rag_system.py')
        pubmed_downloader_path = os.path.join(scripts_path, 'agent_pubmed_downloader.py')
        
        if os.path.exists(rag_system_path):
            spec = importlib.util.spec_from_file_location("agent_rag_system", rag_system_path)
            agent_rag_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(agent_rag_module)
            AgentRAGSystem = agent_rag_module.AgentRAGSystem
        else:
            AgentRAGSystem = None
            
        if os.path.exists(pubmed_downloader_path):
            spec = importlib.util.spec_from_file_location("agent_pubmed_downloader", pubmed_downloader_path)
            agent_pubmed_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(agent_pubmed_module)
            AgentPubMedDownloader = agent_pubmed_module.AgentPubMedDownloader
        else:
            AgentPubMedDownloader = None
    except Exception as e:
        logger.warning("Could not import agent_rag_system or agent_pubmed_downloader: %s", e)
        AgentRAGSystem = None
        AgentPubMedDownloader = None
...
class HepatotoxicityAgent:
    ...
    def __init__(self, model_names=None, use_rag=True, rag_top_k=5, email=None, api_key=None):
        ...
        if use_rag:
            try:
                ...
                logger.warning("Could not initialize RAG system: %s", e)
    ...
    def _download_chemical_abstracts(self, chemical_name: str, max_results=200):
        ...
        if not self.pubmed_downloader:
            logger.warning("PubMed downloader not initialized")
            return []
        
        # Check cache first
        if chemical_name in self.chemical_abstracts_cache:
            logger.info("Using cached abstracts for %s", chemical_name)
            return self.chemical_abstracts_cache[chemical_name]
        
        logger.info("Downloading PubMed abstracts for %s...", chemical_name)
        ...
        if abstracts:
            ...
            logger.info("Generating embeddings for %s new abstracts...", len(abstracts))
            self.rag_system.generate_embeddings(start_index=start_index)
            
            logger.info("RAG system updated with %s abstracts for %s", len(abstracts), chemical_name)
...
Respond in a clear, structured manner that helps users understand how their chemical or data maps to these Key Characteristics."""
    ...
    def _query_model(self, model_name: str, messages: List) -> Optional[str]:
        ...
        try:
            ...
            logger.error("Error with model %s: %s", model_name, e)
            return None
    ...
    def download_pubmed_abstracts(self, max_results=1000):
        ...
        """
        if not self.pubmed_downloader:
            logger.error("PubMed downloader not initialized")
            return
        ...
        if abstracts:
            ...
            logger.info("No abstracts downloaded. Please check your query or network connection.")


def main():
    """Main execution loop for the CLI chatbot."""
    logger.info("=" * 70)
    logger.info("Hepatotoxicity Key Characteristics (KCs) Expert Agent")
    logger.info("Enhanced with RAG and Multi-Model Ensemble")
    logger.info("Based on the framework by Rusyn et al.")
    logger.info("=" * 70)
    logger.info("This agent will help you classify chemicals based on 12 Key Characteristics.")
    logger.info("The agent will automatically download PubMed abstracts for chemicals you query.")
    logger.info("Commands:")
    logger.info("  - Type your question (e.g., 'predict the toxicity of PFOA')")
    ...
    for key_file in api_key_files:
        ...
        if os.path.exists(key_path):
            try:
                ...
                logger.warning("Could not read API key from %s: %s", key_file, e)
    ...
    if not api_key:
        ...
        if api_key_input:
            ...
            logger.info("API key provided. Using higher rate limits.")
        else:
            logger.info("No API key provided. Using standard rate limits.")
            logger.info(
                "Tip: Create a file named 'APIKEY', '.ncbi_api_key', or 'ncbi_api_key.txt' "
                "in this folder to auto-load your API key."
            )
    else:
    ...
    try:
        ...
        logger.error("Please ensure Ollama is running and models are available.")
        return
    
    logger.info(
        "RAG system ready! The agent will automatically download PubMed abstracts "
        "for chemicals mentioned in your queries."
    )
    ...
    while True:
        try:
            ...
            if user_input.lower() in ['exit', 'quit', 'q']:
                logger.info("Thank you for using the Hepatotoxicity KC Expert Agent. Goodbye!")
                break
