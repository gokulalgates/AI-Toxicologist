...
from exceptions import LLMTimeoutError, LLMError


class LLMCallTimeoutError(Exception):
    """Timeout exception for LLM calls (named to avoid shadowing builtin TimeoutError)"""
    pass


def timeout_handler(signum, frame):
    """Signal handler for timeout"""
    raise LLMCallTimeoutError("LLM call timed out")


def invoke_with_timeout(llm, messages, timeout_seconds: float):
    """
    ...
    """
    # Use threading.Timer for cross-platform timeout (signal only works on Unix)
    import threading
