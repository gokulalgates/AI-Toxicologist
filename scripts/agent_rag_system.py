"""
Enhanced RAG (Retrieval-Augmented Generation) system for hepatotoxicity agent.
Uses Ollama embeddings for semantic search over PubMed abstracts.
"""
...
# Try to import ollama, fallback gracefully
try:
    import ollama
...
class AgentRAGSystem:
    ...
    def _ensure_embedding_model(self):
        ...
        if not OLLAMA_AVAILABLE:
            print("Ollama not available, falling back to keyword-based search")
    ...
    def generate_embeddings(self, start_index=0):
        ...
        """
        if not self.use_embeddings or not OLLAMA_AVAILABLE:
    ...
    def retrieve_relevant_abstracts(self, query: str) -> List[Dict]:
        ...
        if not self.use_embeddings or self.embeddings is None or not OLLAMA_AVAILABLE:
